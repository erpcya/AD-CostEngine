...
//	@Override
	...
	public boolean isDelivered()
	{
		...
		for(MPPOrderNode node : this.getMPPOrderWorkflow().getNodes(true, getAD_Client_ID()))
		{
			...
			if(node.getDurationReal().compareTo(Env.ZERO)==1)
			{
				return true;
			}
		}
	...
	public final void createStandardCosts()
	{
		...
		for (MPPOrderNode node : getMPPOrderWorkflow().getNodes(true))
		{
			...
			CostDimension d = new CostDimension(resourceProduct, as, as.getM_CostType_ID(),
					...
					CostDimension.ANY);
			Collection<MCost> costs = d.toQuery(MCost.class, get_TrxName()).list();
			BigDecimal duration = new BigDecimal(node.getDuration());
			String durationUnit = node.getPP_Order_Workflow().getAD_Workflow().getDurationUnit();
			if (durationUnit.equals(X_AD_Workflow.DURATIONUNIT_Minute))
				duration = MUOMConversion.convert(103, resourceProduct.getC_UOM_ID(), duration, false);
			else if (durationUnit.equals(X_AD_Workflow.DURATIONUNIT_Hour))
					duration = MUOMConversion.convert(101, resourceProduct.getC_UOM_ID(), duration, false);
			for (MCost cost : costs)
			{
				//Create or Update the Order Cost dimension
				MPPOrderCost.createOrderCostDimension(get_ID(), cost);
