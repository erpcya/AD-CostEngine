...
//	private final CLogger log = CLogger.getCLogger(getClass());
	...
	public BigDecimal calculateDuration(I_AD_Workflow wf, I_S_Resource plant, BigDecimal qty)
	{
		...
		for (I_AD_WF_Node node : nodes)
		{
			...
			// SHW: consider for "Dedicate Repetitive Flow" the "velocity": how many units you produce in how much time
			BigDecimal durationBeforeOverlap;
			if ( ((MWorkflow)wf).getProcessType().equalsIgnoreCase(MWorkflow.PROCESSTYPE_DedicateRepetititiveFlow) 
					&& node.getUnitsCycles().signum()==1)
				durationBeforeOverlap = new BigDecimal(node.getDuration()).multiply(new BigDecimal(overlapUnits)).divide(node.getUnitsCycles(), 5, BigDecimal.ROUND_UP);
			else
				durationBeforeOverlap = new BigDecimal( node.getDuration() * overlapUnits);
			
			durationTotal += durationBeforeOverlap.doubleValue();
		}
