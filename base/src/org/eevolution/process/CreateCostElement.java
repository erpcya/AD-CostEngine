...
public class CreateCostElement extends SvrProcess
{
			...
			{
				{
					...
					{
						CostDimension d = new CostDimension
								(getAD_Client_ID(), org_id,0,
								product_id, 0, // ASI=0
	...
	private int[] getOrgs(MAcctSchema as, int product_id)
	{
		...
		MProductCategoryAcct pca = MProductCategoryAcct.get(getCtx(), product.getM_Product_Category_ID(), as.get_ID(), 
				product.getAD_Org_ID(),
				get_TrxName());
		
		String CostingLevel = pca.getCostingLevel();
		if (CostingLevel == null || CostingLevel == "")
		...
		else if (MAcctSchema.COSTINGLEVEL_Organization.equals(CostingLevel))
		{
			...
			{
				whereClause = " ad_org_ID in (select ad_org_ID from m_warehouse)";
				return new Query(getCtx(), MOrg.Table_Name,  whereClause, get_TrxName())
