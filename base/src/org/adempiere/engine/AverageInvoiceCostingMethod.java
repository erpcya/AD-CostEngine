...
public class AverageInvoiceCostingMethod extends AbstractCostingMethod
	...
	{	
		...
		if (transaction.getMovementType().endsWith("+"))
		{
			...
			if (model instanceof MMovementLine && costingLevel.equals(MAcctSchema.COSTINGLEVEL_Organization))
			{
				MMovementLine ml = (MMovementLine)model;
				String whereClause = "m_movementline_ID=? and m_transaction_ID <>?";
				ArrayList<Object> params = new ArrayList<Object>();
				params.add(ml.getM_MovementLine_ID());
				params.add(transaction.getM_Transaction_ID());
				MCostDetail cd = new Query(model.getCtx(), MCostDetail.Table_Name, whereClause, model.get_TrxName())
					.setParameters(params)
					.first();
				currentCostPrice = cd.getCurrentCostPrice();
                currentCostPriceLowerLevel = cd.getCurrentCostPriceLL();
